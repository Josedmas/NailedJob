...
export const JobPostingSchema = z.object({
  ...
  link: z.string().url().describe('A URL link to the full job posting or a search results page on the portal.'),
  ...
  async ({ keywords, country, jobPortals, limit }) => {
    ...
    const getPortalSearchLink = (portal: string): string => {
      ...
      if (lowerPortal.includes("infojobs")) {
        return `https://www.infojobs.net/jobsearch/search-results/list.xhtml?keyword=${encodedKeywords}&normalizedProvinceKeyName=&provinceName=`;
      }
